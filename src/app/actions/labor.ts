...
/**
 * 공수관리 표의 셀 하나(날짜 x 공정 x 기공/조공)를 저장한다. 별도의 공정 목록
 * 테이블이 없기 때문에, 0 이하로 지우면 행 자체를 삭제해서 "빈 칸"으로 되돌린다.
 */
export async function saveLaborCell(
  ...
  const { work_date, process_name, worker_type, worker_count } = parsed.data;
  ...
  const { data, error } = await supabase
    .from("labor_records")
    .upsert(
      { project_id: projectId, work_date, process_name, worker_type, worker_count },
      { onConflict: "project_id,work_date,process_name,worker_type" },
    )
    .select()
    .single();
...
/** 공정 삭제 — 해당 공정의 모든 날짜 · 기공/조공 기록을 한 번에 지운다. */
